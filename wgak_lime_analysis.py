...
def main(args):
    ...
    # train_occupancy: (5000, seq_len, node_num) 或 (5000, node_num) 或 (5000, seq_len)
    x_data = train_occupancy[:args.num_samples * args.seq_len]
    occ_data = train_price[:args.num_samples * args.seq_len]
    print("原始x_data.shape:", x_data.shape)
    print("原始occ_data.shape:", occ_data.shape)

    seq_len = args.seq_len
    if x_data.ndim == 3:
        # (num_samples * seq_len, seq_len, node_num) -> (num_samples, node_num, seq_len)
        N = args.num_samples
        node_num = x_data.shape[-1]
        x_data = x_data.reshape(N, seq_len, node_num)
        occ_data = occ_data.reshape(N, seq_len, node_num)
        x_data = np.transpose(x_data, (0, 2, 1))
        occ_data = np.transpose(occ_data, (0, 2, 1))
    elif x_data.ndim == 2:
        # (num_samples * seq_len, node_num)
        N = args.num_samples
        node_num = x_data.shape[1]
        x_data = x_data.reshape(N, seq_len, node_num)
        occ_data = occ_data.reshape(N, seq_len, node_num)
    ...
    plt.tight_layout()
    plt.savefig('./analysis/lime_global_node_importance.png')
    plt.show()
    ...
    heatmap_img.save('./analysis/lime_global_node_spatial_heatmap_pyheatmap.png')
    # 用matplotlib显示
    ...
    plt.tight_layout()
    plt.savefig('./analysis/lime_global_time_importance.png')
    plt.show()
